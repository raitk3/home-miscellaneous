...
text_colors = ["Black", "Black", "White", "Black", "White"]
############################################################
...
class TicketToRide:
    ...
    def longest_railroad(self):
        ...
        for i, player in enumerate(self.list_of_players):
            tk.Button(self.root, text=player.name, fg = text_colors[player.color], bg = header_colors[player.color], command = lambda i=i: self.grande_finale(i)).grid(row=1, column=i, sticky="news")
        self.root.mainloop()
